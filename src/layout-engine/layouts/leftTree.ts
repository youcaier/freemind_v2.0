...
/** 左树：根节点在右侧，所有子节点统一向左展开 */
export function layoutLeftTree(node: WorkingNode, startX: number, startY: number, theme: Theme): LayoutBox {
  measureNode(node, theme);
  return layoutDirectionalSubtree(node, startX, startY, theme, 'left');
}
